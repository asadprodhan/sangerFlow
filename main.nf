...
process blastn {
	...
	output:
    tuple val(query_id), path ("${query.baseName}_blast_sort_topHits.tsv"), emit: topHits
    tuple val(query_id), path ("${query.baseName}_blast_sort.tsv"), emit: allHits
    ...
    head -n ${params.topHits} ${query.baseName}_blast_sort.tsv > ${query.baseName}_blast_sort_topHits.tsv 
...
workflow {
    ...
    blastHits_concat(blastn.out.topHits.collect())
}
